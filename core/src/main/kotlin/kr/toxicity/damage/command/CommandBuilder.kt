...
class CommandBuilder(
    ...
    private companion object {
        ...
        val prefix = listOf(
            emptyComponentOf(),
            "------ BetterDamage ${PLUGIN.semver()} ------".toComponent(GRAY),
            emptyComponentOf()
        )
            ...
            listOf(
                componentOf {
                    ...
                    append("[Wiki]".toComponent {
                        color(AQUA)
                        toURLComponent("https://github.com/toxicity188/BetterDamage/wiki")
                    })
                    append(spaceComponentOf())
    ...
    private fun createBuilder(): Command.Builder<CommandSender> = parent?.createBuilder()?.mapInfo(info) ?: context.manager.commandBuilder(
        info.name,
        info.description,
